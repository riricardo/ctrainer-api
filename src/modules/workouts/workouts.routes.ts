...
const buildWorkoutRoutes = (container: AppContainer) => {
  ...
  router.get("/", guard, controller.listMy);
  router.post("/", guard, validateCreateWorkout, controller.create);
  router.get("/:id", guard, controller.getById);
  router.put("/:id", guard, validateUpdateWorkout, controller.updateById);
